...
class CPC_word(nn.Module):  ####输入流数据是每一个批次的每个路径的关系，实体，mask和负采样实体
    def __init__(self, ent_vocab, rel_vocab, embedding_size, ar_dim, window, k_size, path_length):
        ...
        # self.ent_embedding = nn.Embedding(self.ent_vocab, self.embedding_size)
        # self.rel_embedding = nn.Embedding(self.rel_vocab, self.embedding_size)
    ...
    def forward(self, ents, rels, device):  # x.shape: (B,W,S) ####输入流数据是每一个批次的每个路径的关系，实体，mask和负采样实体
        ...
        # print(ents.shape)#torch.Size([128, 3])
        ...
        for i in range(self.k_size):
            ...
            pred[i] = linear(context)  # Wk*context.shape: (B,D) pred[i].shape:(B,self.embedding_size),pred.shape: (k_size,B,self.embedding_size)也即（W2，B，D）
    ...
    def get_input_embedding(self, batch, ents, rels):  # batch * ent_num,batch * rel_num
        ...
        #         if len(path_embed) != self.path_length:
        #             # print(i,t)
        #             path_embed.append(rel_embed[i][t])
        #         else:
        #             paths_embed.append(path_embed)
        #             path_embed = []
    ...
    def info_nce(self, prediction, target):
        ...
        loss = nn.functional.cross_entropy(logits, label, reduction='none')   ###不用对label进行one_hot编码，因为nn.functional.cross_entropy函数中的nll_loss函数已经实现了类似one-hot过程，不同之处是当class = [1, 2, 3]时要处理成从0开始[0, 1, 2]。
...
#     # ent_negs = torch.zeros([64, 3, 20]).long().to(device)
#     ent_negs = None
#     # (self, ent_vocab, rel_vocab, embedding_size,ar_dim, window, k_size,path_length)
#     model = CPC(60, 6, 100, 100, 5, 2, 5).to(device)
#     loss, accuracy = model(ents, rels, ent_negs, device)  # forward(self, ents, rels)
