...
def findAllPaths(graph, start, distance,hop_num,flag,path=[]):
    ...
    if flag == 0:
        ...
        for node in x:
            # print(node)
            if node[0] not in path:
                newpaths = findAllPaths(graph, node, distance, hop_num,flag, path)
...
def get_path(graph, hop_num):
    allpaths = []
    l = list(graph.keys())
    for t in tqdm(range(len(l))):
        node = l[t]
        path = findAllPaths(graph, node, 0, hop_num, 0)   #(graph, start, distance,length,flag,path=[])
        if path == []:
            continue
        for p in path:
            allpaths.append(p)
...
if __name__ == '__main__':
   ...
   train_paths_2hop = get_path(graph_train, hop_num=2)
   torch.save(train_paths_2hop, datasets + "train_paths_2hop.pt")
   print(train_paths_2hop[:10], len(train_paths_2hop))
   ###[[0, 0, 1, 1, 5305], [0, 0, 1, 0, 10486], [0, 0, 1, 1, 11107], [0, 5, 5363, 1, 31860], [0, 5, 5363, 1, 6445],
   # [0, 5, 5363, 0, 7240], [1, 1, 5305, 0, 2327], [1, 1, 5305, 1, 1754], [1, 1, 5305, 1, 29442],
   # [1, 1, 5305, 1, 13817]] 174249

   train_paths_5hop = get_path(graph_train, hop_num=5)
   torch.save(train_paths_5hop, datasets + "train_paths_5hop.pt")
   print(train_paths_5hop[:10], len(train_paths_5hop))
   #[[0, 0, 1, 1, 5305, 0, 2327, 1, 22113, 0, 10486], [0, 0, 1, 1, 5305, 0, 2327, 1, 22113, 1, 10662],
   # [0, 0, 1, 1, 5305, 0, 2327, 1, 10486, 0, 8404], [0, 0, 1, 1, 5305, 0, 2327, 1, 16612, 1, 10662],
   # [0, 0, 1, 1, 5305, 0, 2327, 1, 17379, 1, 17378], [0, 0, 1, 1, 5305, 0, 2327, 1, 17379, 0, 4865],
   # [0, 0, 1, 1, 5305, 1, 1754, 1, 27418, 0, 18054], [0, 0, 1, 1, 5305, 1, 1754, 1, 8152, 1, 29442],
   # [0, 0, 1, 1, 5305, 1, 1754, 1, 8152, 0, 239], [0, 0, 1, 1, 5305, 1, 1754, 1, 8152, 1, 8151]] 1982792



   # valid_paths = get_path(graph_valid)
   # torch.save(valid_paths, datasets + "valid_paths.pt")
   #
